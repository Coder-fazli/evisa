...
async function getPage(locale: string) {
  ...
  try {
    const data = await client.fetch(
      `*[_type == "infoPage" && slug.current == "requirements"][0] {
        "title": ${fields.title},
        "body": ${fields.body},
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const currentUrl = `${baseUrl}/${locale}/requirements`;

  return {
    title: page?.metaTitle ?? page?.title ?? "Visa Requirements",
    description: page?.metaDescription ?? "",
    alternates: {
      languages: {
        en: `${baseUrl}/en/requirements`,
        es: `${baseUrl}/es/requirements`,
        ar: `${baseUrl}/ar/requirements`,
      },
...
export default async function RequirementsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const page = await getPage(locale);
